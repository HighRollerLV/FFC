<div class="whiteLine w-full bg-white h-[5px] my-6"></div>
<footer class="w-full h-[25vh] flex flex-row">
    <div class="flex-wrap flex flex-row w-[100%] h-[15vh] pt-16">
        <div class="media flex flex-col w-[15%] h-[15vh] text-left gap-12">
            <a href="http://into.id.lv/ip19/ralfs/galadarbshelp/index.php"
               class="w-[100%] md:w-[70%] lg:w-[50%] xl:w-[30%]"><img src="includes/images/FFCblack.png"></a>
            <div class="flex flex-row text-[#F9FAFA]">
                <a href="#"><i class="uil uil-facebook-f text-2xl hover:text-[#e4c065]"></i></a>
                <a href="#"><i class="uil uil-instagram text-2xl hover:text-[#e4c065]"></i></a>
                <a href="#"><i class="uil uil-twitter text-2xl hover:text-[#e4c065]"></i></a>
                <a href="#"><i class="uil uil-discord text-2xl hover:text-[#e4c065]"></i></a>
            </div>
        </div>
        <div class="support flex flex-col w-[25%]  text-left text-[#F9FAFA]">
            <a href="#"
               class="text-base md:text-lg hover:text-[#e4c065] underline-none hover:underline hover:underline-offset-8 w-[21%] hover:transition duration-300 ease-out hover:scale-110">Contact
                Us</a>
            <a href="#"
               class="text-base md:text-lg hover:text-[#e4c065] underline-none hover:underline hover:underline-offset-8 w-[7%] hover:transition duration-300 ease-out hover:scale-110">Faq</a>
            <a href="#"
               class="text-base md:text-lg hover:text-[#e4c065] underline-none hover:underline hover:underline-offset-8 w-[9%] hover:transition duration-300 ease-out hover:scale-110">Q&A</a>
        </div>
        <div class="ffcLinks flex flex-col w-[20%]  text-left text-[#F9FAFA]">
            <a href="http://into.id.lv/ip19/ralfs/galadarbshelp/index.php"
               class="text-base md:text-lg hover:text-[#e4c065] underline-none hover:underline hover:underline-offset-8 w-[10%] hover:transition duration-300 ease-out hover:scale-110">Home</a>
            <a href="http://into.id.lv/ip19/ralfs/galadarbshelp/rankings.php"
               class="text-base md:text-lg hover:text-[#e4c065] underline-none hover:underline hover:underline-offset-8 w-[8%] hover:transition duration-300 ease-out hover:scale-110">Ranks</a>
            <a href="http://into.id.lv/ip19/ralfs/galadarbshelp/bets.php"
               class="text-base md:text-lg hover:text-[#e4c065] underline-none hover:underline hover:underline-offset-8 w-[6%] hover:transition duration-300 ease-out hover:scale-110">Bets</a>
            <a href="http://into.id.lv/ip19/ralfs/galadarbshelp/info.php"
               class="text-base md:text-lg hover:text-[#e4c065] underline-none hover:underline hover:underline-offset-8 w-[5%] hover:transition duration-300 ease-out hover:scale-110">Info</a>
            <a href="http://into.id.lv/ip19/ralfs/galadarbshelp/results.php"
               class="text-base md:text-lg hover:text-[#e4c065] underline-none hover:underline hover:underline-offset-8 w-[10%] hover:transition duration-300 ease-out hover:scale-110">Results</a>
            <a href="http://into.id.lv/ip19/ralfs/galadarbshelp/profile.php"
               class="text-base md:text-lg hover:text-[#e4c065] underline-none hover:underline hover:underline-offset-8 w-[10%] hover:transition duration-300 ease-out hover:scale-110">Profile</a>
        </div>
        <div class="updateSignup flex flex-col w-[35%] text-left">
            <p class="text-[#F9FAFA] text-lg md:text-xl">Sign up for Updates</p>
            <input type="text" class="w-[100%] xl:w-[40%] lg:w-[60%] sm:w-[80%] outline-none"
                   placeholder="Enter your e-mail...">
            <button class="text-[#F9FAFA] bg-[#959595] w-[60%] lg:w-[30%] hover:bg-[#e4c065] hover:text-[#F9FAFA] hover:transition duration-300 ease-out hover:scale-110">
                Send
            </button>
        </div>
        <div>
</footer>
</body>

<script src="includes/js/slider.js"></script>
<script src="includes/js/script.js"></script>
